...
└─────────────┼───────────────────────────────────────────────────┘
    ...
    │  │                                                  │   │
    │  │  ┌──────────────────────────────────────────┐   │   │
    │  │  │  health-ledger         (host network)     │   │   │
    │  │  │  health-ledger-tunnel  (host network)     │   │   │
    │  │  └──────────────────────────────────────────┘   │   │
    │  │                                                  │   │
    │  │  ┌──────────────────────────────────────────┐   │   │
    │  │  │  archi-mcp-bridge         (host network)  │   │   │
...
Each per-service tunnel has its hostname routing configured in the **Cloudflare dashboard** (not in local config files). A `TUNNEL_TOKEN` is generated from the dashboard, stored in GitHub repo secrets as `CF_TUNNEL_TOKEN`, and injected at deploy time via the `docker-compose.override.yml` (see §10.3).
...
/opt/
├── remarkable-pipeline/
│   ├── docker-compose.yml          (non-secret config — SCP'd from repo by CI/CD)
│   ├── docker-compose.override.yml (secrets in environment: blocks — generated by CI/CD)
...
Note: All three services use host networking. Container ports (8766, 8765, 8767) are bound to the host network stack and accessible to their respective cloudflared sidecars at localhost.
...
**Environment variables** (non-secret, in base compose):
- `MCP_TRANSPORT=streamable-http` — server transport mode
- `HTTP_PORT=8766` — binding port
- `HTTP_HOST=0.0.0.0` — binding host (all interfaces)
- `HTTP_BEARER_TOKEN_REQUIRED=false` — auth not enforced
...
**Secrets** (in CI/CD-generated override):
- `HTTP_BEARER_TOKEN` — API authentication key
- `OPENAI_API_KEY` — OpenAI API key for OCR prompts
- `RM_CONNECT_TOKEN` — reMarkable Connect cloud API token
- `TUNNEL_TOKEN` — Cloudflare tunnel token (cloudflared service)
...
**Port**: 8766 (MCP streamable HTTP) — bound to host network (0.0.0.0)
...
| Container | Image | Role |
|---|---|---|
| `health-ledger` | `ghcr.io/matthewdart/health-ledger:latest` | MCP server — Apple Health data ingestion, SQL analytics |
| `health-ledger-tunnel` | `cloudflare/cloudflared:latest` | Cloudflare tunnel sidecar |

**Environment variables** (non-secret, in base compose):
- `HTTP_PORT=8765` — binding port
- `HTTP_HOST=127.0.0.1` — binding host
- `HTTP_BEARER_TOKEN_REQUIRED=true` — auth enforced
- `HEALTH_LEDGER_TRANSPORT=mcp` — server transport mode
- `HLA_INPUT_ZIP=/app/data/input/export.zip` — health export input path
- `HLA_OUTPUT_DIR=/app/data/output` — output directory
...
**Port**: 8765 (MCP streamable HTTP) — `127.0.0.1:8765`

**Cloudflare route**: `health-ledger-mcp.matthewdart.name` → port 8765
...
| `archi-mcp-bridge-tunnel` | `cloudflare/cloudflared:latest` | Cloudflare tunnel sidecar |

**Environment variables** (non-secret, in base compose):
- `HTTP_PORT=8767` — binding port
- `HTTP_HOST=127.0.0.1` — binding host
- `HTTP_BEARER_TOKEN_REQUIRED=false` — auth not enforced
- `ARCHI_MCP_FIXTURE_MODEL=/data/input/models/minimal.archimate` — default ArchiMate model
- `ARCHI_MCP_WORKSPACE=/data/workspace` — Archi workspace path
...
**Port**: 8767 (HTTP) — `127.0.0.1:8767`

**Cloudflare route**: `archi-mcp.matthewdart.name` → port 8767
...
```
┌─────────────────────────────────────────────────────────────┐
│  HOST NETWORK (network_mode: host — all services)           │
│                                                             │
│   remarkable-pipeline-mcp (:8766, 0.0.0.0)                  │
│   remarkable-pipeline-tunnel                                │
│   (can reach Tailscale IPs: 100.x.x.x)                     │
│                                                             │
│   health-ledger (:8765, 127.0.0.1)                          │
│   health-ledger-tunnel                                      │
│                                                             │
│   archi-mcp-bridge (:8767, 127.0.0.1)                       │
│   archi-mcp-bridge-tunnel                                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘

All services use host networking. No cross-service Docker networking.
Each cloudflared sidecar reaches its app at localhost:<port>.
...
#### `deploy-stack.yml`
Deploys a service to the Oracle VM via Tailscale SSH. Supports secrets injection via override templates.

```
Trigger: workflow_call from project repos (with secrets: inherit)
Inputs:
  - service_name (required) — deploys to /opt/<service_name>/
  - compose_file (default: docker-compose.yml)
  - compose_override_template (default: docker-compose.override.deploy.yml)
  - vm_hostname (default: matthews-oracle-instance)
  - vm_user (default: ubuntu)
Steps:
  1. Checkout calling repo
  2. Generate docker-compose.override.yml from template via envsubst
     (expands ${PLACEHOLDER} syntax with secrets from GitHub Actions env)
  3. Connect to Tailscale (ephemeral auth key)
  4. SSH: mkdir -p /opt/<service>/ on VM
  5. SCP: docker-compose.yml → /opt/<service>/docker-compose.yml
  6. SCP: docker-compose.override.yml → /opt/<service>/docker-compose.override.yml
     (only if override was generated from template)
  7. SSH: cd /opt/<service>/ && docker compose pull && docker compose up -d
  8. SSH: docker compose ps (verify)
```

Docker Compose auto-merges `docker-compose.yml` + `docker-compose.override.yml`. The override injects secrets directly as `environment:` entries — no `.env` file needed in the deploy path.
...
Calling repos use `secrets: inherit` to pass all repository secrets to the reusable workflows.
...
| `RM_CONNECT_TOKEN` | remarkable-pipeline | deploy-stack.yml → override | reMarkable Connect cloud API token |
| `GITHUB_TOKEN` | all (auto) | build-arm-image.yml | Auto-provided; authenticates to GHCR |
...
Developer pushes to main
       │
       ▼
GitHub Actions triggers docker.yml (secrets: inherit)
       │
       ...
       └──► Job 2: deploy-stack (depends on Job 1)
                 │
                 ├── Checkout code (for compose files + override template)
                 ├── Generate docker-compose.override.yml:
                 │     envsubst < docker-compose.override.deploy.yml
                 │     (expands ${SECRET} placeholders with GitHub secrets)
                 ├── Join Tailscale mesh (ephemeral key)
                 ├── SSH to matthews-oracle-instance:
                 │     mkdir -p /opt/<service>/
                 ├── SCP docker-compose.yml to /opt/<service>/
                 ├── SCP docker-compose.override.yml to /opt/<service>/
                 │     (secrets in environment: blocks — never committed)
                 ├── SSH: docker compose pull
                 │     (pulls new image from GHCR)
                 ├── SSH: docker compose up -d
                 │     (compose auto-merges base + override)
...
| Location | Contents |
|---|---|
| `/opt/<service>/docker-compose.yml` | Service definitions + non-secret config. Committed artefact, SCP'd from repo. No `env_file:`, no `${VAR}` interpolation. |
| `/opt/<service>/docker-compose.override.yml` | Secrets in `environment:` blocks. Generated by CI/CD from `docker-compose.override.deploy.yml` template via `envsubst`. Overwritten each deploy. Not committed. |
...
| 7 | health-ledger | Docker container | VM (host net) | TCP 8765 | Health data MCP server |
| 8 | health-ledger-tunnel | Docker container | VM (host net) | — | Cloudflare tunnel for health-ledger |
| 9 | archi-mcp-bridge | Docker container | VM (host net) | TCP 8767 | ArchiMate MCP bridge |
| 10 | archi-mcp-bridge-tunnel | Docker container | VM (host net) | — | Cloudflare tunnel for archi-mcp-bridge |
...
| **Secrets injection** | Stored in GitHub repo secrets. Injected at deploy time via `docker-compose.override.deploy.yml` template → `envsubst` → `docker-compose.override.yml` on VM. Secrets appear only in the override file on disk. Never in committed artefacts or images. |
